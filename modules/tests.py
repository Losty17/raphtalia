...
watcher = LolWatcher('RGAPI-a785222c-b96b-414e-91e4-874a0115c17b')
lol_region = 'br1'
...
class Tests(commands.Cog):
    ...
    # @commands.command(aliases=['reputação','reputation'])
    # #@commands.cooldown(1, 10800, commands.BucketType.user)
    # async def rep(self, ctx, member : discord.Member = None):
    #     if member is None: return await ctx.send('Uso: `rep @usuário`.')
    #     if member is ctx.author: return await ctx.send('Você não pode dar reputação para si mesmo!')
    #     get = avatar_collection.find_one({'_id': member.id})
    #     if get is None: return await ctx.send('Este usuário não se registrou!')
    #     rep = get['rep']
    #     rep += 1
    #     avatar_collection.update_one({'_id': member.id}, {'$set': {'rep': rep}})
    #     await ctx.send(f'Oba! Você acabou de dar mais reputação para {member.mention}! <:sleepolfo:697924312145068092>')
    #     @commands.cooldown(1, 10800, commands.BucketType.user)
